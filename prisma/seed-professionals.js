...
function classify(roleRaw){const k=deaccent(roleRaw).toLowerCase();for(const [re,name] of SPEC){if(re.test(k))return{kind:'ESPECIALISTA',specialty:name,title:roleRaw||name};}return{kind:'EQUIPE',specialty:null,title:roleRaw||'Equipe Clinicat'};}

async function main() {
  let equipeOrder = 0, espOrder = 0;
  for (const { file, webp } of PHOTOS) {
    const { name, roleRaw } = parseFilename(file);
    const c = classify(roleRaw);
    const meta = META[slugifyName(name)] || {};
    const photoUrl = `/team/${webp}`;
    const displayOrder = c.kind === 'ESPECIALISTA' ? espOrder++ : equipeOrder++;
    // upsert idempotente por (name+photoUrl) — usamos photoUrl como chave estável de seed
    const existing = await prisma.professional.findFirst({ where: { photoUrl } });
    const data = {
      name, kind: c.kind, title: c.title, specialty: c.specialty,
